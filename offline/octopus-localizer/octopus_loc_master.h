...
/* a.k.a. "The Source Localization Engine"..
    This is the predefined parameters, configuration and management class
    shared over all other classes of octopus-localizer. */

#ifndef OCTOPUS_LOC_MASTER_H
#define OCTOPUS_LOC_MASTER_H
...
class LocMaster : QObject {
 Q_OBJECT
 public:
  LocMaster(QApplication *app) : QObject() { application=app;
   ...
   hwFrameV=hwGridV=hwScalpV=hwParamV=hwRealV=hwGizmoV=hwAvgsV=true;
...
#include "rm_parse.h" // Will be transferred to external method file..
  ...
  void loadScalp_ObjFile(QString fn) {
   ...
   while (!scalpStream.atEnd()) {
    ...
    if (dummySL[0]=="v") {
     ...
     dummySL2=dummySL[1].split("/"); idx.append(dummySL2[0].toInt()-1);
     dummySL2=dummySL[2].split("/"); idx.append(dummySL2[0].toInt()-1);
     dummySL2=dummySL[3].split("/"); idx.append(dummySL2[0].toInt()-1);
  ...
  void loadReal(QString fileName) {
   ...
   for (int i=0;i<realValidLines.size();i++) {
    ...
    if (opts.size()==8 && opts[0]=="v") { opts.removeFirst();
     p=opts[0].toInt(); c=-1;
...
//     acqChannels[chnIdx]->real[0]=opts[1].toFloat();
//     acqChannels[chnIdx]->real[1]=opts[2].toFloat();
//     acqChannels[chnIdx]->real[2]=opts[3].toFloat();
//     acqChannels[chnIdx]->realS[0]=opts[4].toFloat();
//     acqChannels[chnIdx]->realS[1]=opts[5].toFloat();
//     acqChannels[chnIdx]->realS[2]=opts[6].toFloat();
    } else {
  ...
  // Gizmo
  QVector<Gizmo* > gizmo; bool gizmoOnReal;
  ...
  bool hwFrameV,hwGridV,hwScalpV,hwParamV,hwRealV,hwGizmoV,hwAvgsV,
       scalpExists,gizmoExists;
 ...
 signals:
  void repaintGL(int); void repaintHeadWindow();
...
/*
   ...
   avgFileName="rec_"+currentDT.toString("dd-MM-yyyy-hh-mm-ss-zzz");
   for (int i=0;i<eventIndex.size();i++) {
    avgInfoFile.setFileName(avgFileName+"-"+
                           dummyString.setNum(eventIndex[i])+".inf");
    avgDataFile.setFileName(avgFileName+"-"+
                           dummyString.setNum(eventIndex[i])+".avg");
    if (!avgInfoFile.open(QIODevice::WriteOnly | QIODevice::Text) ||
        !avgDataFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
     qDebug("Error: Cannot open .avg or .inf file for writing.");
     return;
    }
    avgInfoStream.setDevice(&avgInfoFile);
    avgDataStream.setDevice(&avgDataFile);
    avgDataStream.setRealNumberPrecision(4);

    avgInfoStream << 0.7 << "\n";	    // Version
    avgInfoStream << SRATE << "\n";	    // SampleRate
    avgInfoStream << eegBwdRejSize << "\n"; // Backward Rej Sample#
    avgInfoStream << eegBwdAvgSize << "\n"; // Backward Avg Sample#
    avgInfoStream << eegFwdAvgSize << "\n"; // Forward Avg Sample#
    avgInfoStream << eegFwdRejSize << "\n"; // Forward Rej Sample#
    avgInfoStream << rejThreshold << "\n";  // 0 -> +-Peak Rejection Threshold
    avgInfoStream << eventCounts[i] << "\n"; // Total Events
    avgInfoStream << rejectedCounts[i] << "\n"; // Rejected Events
    
    for (int j=0;j<(eegBwdAvgSize+eegFwdAvgSize);j++) { // in uV
     for (int k=0;k<N_CHNS;k++) // in uV
      avgDataStream << (*(*(avgData[i]))[k])[j] << " ";
     for (int k=0;k<N_CHNS;k++) // in uV
      avgDataStream << (*(*(avgSData[i]))[k])[j] << " ";
     avgDataStream << "\n";
    }
    avgInfoFile.close();
    avgDataFile.close();
...
//      scrPrvData[i]=scrCurData[i]; scrCurData[i]=acqCurData[1+nChns+i];
    ...
    if (!seconds) emit repaintGL(2); // Update 50Hz visualization..
...
*/
 ...
 private: // Used Just-In-Time..
  ...
  QVector<float> calM,calN;
  int cntBufIndex,scrCounter,seconds; bool tick,event;
  QString pattern,dummyString;
