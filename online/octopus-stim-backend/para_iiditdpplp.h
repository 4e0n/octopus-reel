  ...
  if (counter2==0) {
   ...
   switch (current_pattern_data) {
    ...
    case '0': para_iiditdpplp_current_iid_amp_l=DA_NORM; /* ITD=0,IID: */
              ...
              para_iiditdpplp_itd_lr_delta=0;
              if (trig_active) outb(0x80|SEC_PP_CENTER,0x378);
              break;
...
//   stim_reset();
   ...
   comedi_data_write(pcimio,1,0,0,AREF_GROUND,dac_0);
   comedi_data_write(pcimio,1,1,0,AREF_GROUND,dac_1);
   comedi_data_write(pcimio,1,2,0,AREF_GROUND,dac_2);
  ...
  pause_trighi++;
  if (pause_trighi==TRIG_HI_STEPS) outb(0x00,0x378);
 }
 dac_2=dac_3=0;
}
