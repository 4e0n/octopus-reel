...
#ifdef OCTOPUS_STIM_COMEDI
    comedi_data_write(pcimio,1,0,0,AREF_GROUND,DACZERO+dac_0); // L
    comedi_data_write(pcimio,1,1,0,AREF_GROUND,DACZERO+dac_1); // R
    comedi_data_write(pcimio,1,2,0,AREF_GROUND,DACZERO+dac_2); // X
    comedi_data_write(pcimio,1,3,0,AREF_GROUND,DACZERO+dac_3); // Y
#endif
...
int fbfifohandler(unsigned int fifo,int rw) {
 ...
 "octopus-stim-backend.o: Stimulus pattern Xferred to backend successfully.\n");
                           break;
   case STIM_START:        stim_paused=0; stim_active=1;
                 rt_printk("octopus-stim-backend.o: Stim resumed/started.\n");
                           break;
   case STIM_PAUSE:        pause_trighi=0; stim_paused=1;
                           rt_printk("octopus-stim-backend.o: Stim paused.\n");
                           break;
   case STIM_STOP:         stim_active=lp0_data=0;
                           stim_reset();
                           init_test_para(paradigm);
#ifdef OCTOPUS_STIM_COMEDI
                           comedi_data_write(pcimio,1,0,0,AREF_GROUND,dac_0);
                           ...
                           break;
   case TRIG_START:        trig_active=1;
                           rt_printk("octopus-stim-backend.o: Trig started.\n");
                           break;
   case TRIG_STOP:         trig_active=0;
                           rt_printk("octopus-stim-backend.o: Trig stopped.\n");
                           break;
   case STIM_RST_SYN:      stim_active=lp0_data=0;
                           rt_sem_wait(&stim_sem);
                            rtf_reset(BFFIFO); rtf_reset(FBFIFO);
                            stim_reset();
                           rt_sem_signal(&stim_sem);
                        ...
                        rt_printk("octopus-stim-backend.o: Backend reset.\n");
                           break;
   case STIM_SYNTH_EVENT:  outb(0x80|(fb_msg.iparam[0] & 0x7f),0x378);
                           msleep(1);
                           outb(0,0x378);
   default:                break;
  }
...
#ifdef OCTOPUS_STIM_COMEDI
 pcimio=comedi_open("/dev/comedi0"); comedi_lock(pcimio,1);
 comedi_data_write(pcimio,1,0,0,AREF_GROUND,dac_0);
 comedi_data_write(pcimio,1,1,0,AREF_GROUND,dac_1);
 comedi_data_write(pcimio,1,2,0,AREF_GROUND,dac_2);
 comedi_data_write(pcimio,1,3,0,AREF_GROUND,dac_3);
