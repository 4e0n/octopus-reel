...
//#else
/* Due to a momentary problem with RTAI this has to be defined manually. */
...
static int par_base=ACQ_PARBASE,ser_base=ACQ_SERBASE,
	   par_irq=ACQ_PARIRQ,ser_irq=ACQ_SERIRQ;
...
/* ------------------------------------------------------------------------- */
    ...
    if (data_loss_flag) b2f_cmd(MSG_ALERT,ALERT_DATA_LOSS,0,0,0);
...
//#endif
    ...
    b2f_msg.id=CMD_B2F; b2f_msg.iparam[0]=B2F_DATA_SYN;
...
static int fb_fifohandler(unsigned int fifo,int rw) {
 if (rw=='w') {
  ...
  switch (f2b_msg.id) {
   case ACT_START:  reset_fifos(); acq_active=1;
                    rt_printk("octopus-acq-backend.o: Acquisition started.\n");
                    break;
   case ACT_STOP:   acq_active=0;
