...
class EEGThread : public QThread {
 ...
 public:
  explicit EEGThread(ConfParam *c=nullptr,unsigned int a=0,QImage *sb=nullptr,QObject *parent=nullptr) : QThread(parent) {
   ...
   // choose RMS window length (seconds)
   const float windowSec=1.0f; // configurable later
   powWinChunks=int(std::lround(windowSec*float(conf->eegRate)/float(conf->scrUpdateSamples)));
   if (powWinChunks<2) powWinChunks=2;
   powWin.resize(std::size_t(chnCount));
   for (auto& w:powWin) w.init(powWinChunks);
...
//    }
  ...
  void chunkEnergy(unsigned int startIdx,std::vector<float>* e) {
   const unsigned tcpBufTail=conf->tcpBufTail;
   const unsigned scrUpdateSmp=conf->scrUpdateSamples;
   const float invSmp=1.0f/float(scrUpdateSmp);
   ...
   PARFOR(chnIndex,0,int(chnCount)) {
    ...
    (*e)[chnIndex]=float(sumSq*invSmp); // mean square for this chunk
   }
  }
...
;   }
    ...
    for (unsigned int smpIdx=0;smpIdx<scrUpdateCount-1;smpIdx++) {
     ...
     std::vector<float> eChunk(chnCount);
     chunkEnergy((smpIdx+1)*scrUpdateSmp,&eChunk);

     // push chunk energies into running windows
     for (unsigned int ch=0;ch<chnCount;++ch) powWin[std::size_t(ch)].push(eChunk[ch]);

//float rms1s = powWin[ch].rms();   // RMS over ~1 sec (chunked)
//float pwr1s = powWin[ch].meanEnergy(); // mean square over ~1 sec
...
*/
 ...
 private:
  struct RunningChunkEnergy {
   int W=0;            // number of chunks in window
   int idx=0;
   bool filled=false;
   double sumE=0.0;    // sum of chunk energies (mean square)
   std::vector<float> ringE;

   void init(int windowChunks) {
    W=std::max(1,windowChunks);
    idx=0; filled=false;
    sumE=0.0;
    ringE.assign(std::size_t(W),0.0f);
   }

   inline void push(float eChunk) { // eChunk=mean(x^2) over one chunk
    float old=ringE[std::size_t(idx)];
    sumE-=double(old);

    ringE[std::size_t(idx)]=eChunk;
    sumE+=double(eChunk);

    idx++;
    if (idx>=W) { idx=0; filled=true; }
   }

   inline bool ready() const { return filled; }

   inline float meanEnergy() const {
    int denom=filled ? W:std::max(1,idx);
    return float(sumE/double(denom));
   }

   inline float rms() const {
    return std::sqrt(std::max(0.0f,meanEnergy()));
   }
  };

  // node-freq v0
  std::vector<RunningChunkEnergy> powWin; // per channel
  int powWinChunks=0;
