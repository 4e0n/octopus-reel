  ...
  bool initialize() { QString cfgPath;
   ...
   if (QFile::exists(cfgPath)) { ConfigParser cfp(cfgPath);
    if (!cfp.parse(&conf)) {
     ...
     qInfo() << "---------------------------------------------------------------";
     qInfo() << "node-freq: <ServerIP> is" << conf.acqIpAddr;
     qInfo() << "node-freq: <Comm> listening on AcqServ ports (comm,strm):" << conf.acqCommPort << conf.acqStrmPort;
     //qInfo() << "node-freq: <Comm> listening on StorServ ports (comm):" << conf.storCommPort;
     qInfo() << "node-freq: <GUI> Ctrl (X,Y,W,H):" << conf.guiCtrlX << conf.guiCtrlY << conf.guiCtrlW << conf.guiCtrlH;
     qInfo() << "node-freq: <GUI> Amp (X,Y,W,H):" << conf.guiAmpX << conf.guiAmpY << conf.guiAmpW << conf.guiAmpH;

     // ---------------------------------------------------------------------------------------------------------------
     if (!commServer.listen(QHostAddress::Any,conf.freqCommPort)) {
      qCritical() << "node-freq: Cannot start TCP server on <Comm> port:" << conf.freqCommPort;
      return true;
     }
     qInfo() << "node-freq: <Comm> listening on port" << conf.freqCommPort;
     if (!strmServer.listen(QHostAddress::Any,conf.freqStrmPort)) {
      qCritical() << "node-freq: Cannot start TCP server on <Strm> port:" << conf.freqStrmPort;
      return true;
     }
     qInfo() << "node-freq: <Strm> listening on port" << conf.freqStrmPort;
     // ---------------------------------------------------------------------------------------------------------------
     ...
     qWarning() << "node-freq: The config file" << cfgPath << "is corrupt!";
     return true;
    }
   } else {
    qWarning() << "node-freq: The config file" << cfgPath << "does not exist!";
    return true;
   }
  }
 ...
 private slots:
  void onNewCommClient() {
   while (commServer.hasPendingConnections()) {
    ...
    qInfo() << "node-freq: <Comm> Client connected from" << client->peerAddress().toString();
   }
  }
  ...
  void onNewStrmClient() {
   while (strmServer.hasPendingConnections()) {
    ...
    connect(client,&QTcpSocket::disconnected,this,[this,client]() {
     qDebug() << "node-freq: <Strm> client from" << client->peerAddress().toString() << "disconnected.";
     strmClients.removeAll(client);
     client->deleteLater();
    });
    qInfo() << "node-freq: <Strm> client connected from" << client->peerAddress().toString();
   }
  }
