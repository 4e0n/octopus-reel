...
#include <QPainter>
#include "confparam.h"
#include "cmodthread.h"

class CModFrame : public QFrame {
 Q_OBJECT
 public:
  explicit CModFrame(ConfParam *c=nullptr,unsigned int a=0,QWidget *parent=nullptr) : QFrame(parent) {
   conf=c; ampNo=a;
   ...
   cmThread=new CModThread(conf,ampNo,&cmBuffer,this);
   conf->threads[ampNo]=cmThread;
   connect(cmThread,&CModThread::updateCModFrame,this,QOverload<>::of(&CModFrame::update));
   cmThread->start();
  }

  QPixmap cmBuffer; CModThread *cmThread;
