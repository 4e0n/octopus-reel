...
class ConfParam : public QObject {
 ...
 public:
  ...
  void loadGizmo(QString fn) {
   ...
   for (int i=0;i<validLines.size();i++) {
    ...
    if (opts.size()==2 && opts2.size()>0) {
     ...
     if (opts[0].trimmed()=="GIZMO") {
      ...
      if (opts2.size()%3==0) {
       ...
       qDebug() << "hnode_gui: <ConfParam> <LoadGizmo> <OGZ> ERROR: Triangles not multiple of 3 vertices..";
      }
	...
	glGizmo[k].lin.append(ll);
       ...
       qDebug() << "hnode_gui: <ConfParam> <LoadGizmo> <OGZ> ERROR: Lines not multiple of 2 vertices..";
      }
     }
    } else { gError=true; qDebug() << "hnode_gui: <ConfParam> <LoadGizmo> <OGZ> ERROR: while parsing!"; }
  ...
  int guiCtrlX,guiCtrlY,guiCtrlW,guiCtrlH, guiAmpX,guiAmpY,guiAmpW,guiAmpH, ampFrameW,ampFrameH, audWaveH;
  ...
  void onStrmDataReady() {
   ...
   while (buffer.size()>=4) {
    ...
    if (tcpS.deserialize(block,chnCount)) {
     ...
     for (int ampIdx=0;ampIdx<ampCount;ampIdx++) for (int chnIdx=0;chnIdx<chnCount;chnIdx++) {
      float avg;
      if (chns[chnIdx].interActive[ampIdx]) {
       avg=0.0; for (int idx=0;idx<chns[chnIdx].interElec.size();idx++) avg+=tcpS.amp[ampIdx].data[ chns[chnIdx].interElec[idx] ];
       tcpS.amp[ampIdx].data[chnIdx]=avg;
       avg=0.0; for (int idx=0;idx<chns[chnIdx].interElec.size();idx++) avg+=tcpS.amp[ampIdx].dataF[ chns[chnIdx].interElec[idx] ];
