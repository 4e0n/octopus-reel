 ...
 public:
  ...
  bool parse(ConfParam *conf) {
   ...
   if (!cfgFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    qDebug() << "hnode_gui: <ConfigParser> ERROR: Cannot load" << cfgPath;
    return true;
   } else {
    ...
    for (auto& cl:cfgLines) {
     opts=cl.split("#"); opts=opts[0].split("|"); // Get rid off any following comment.
      ...
      qDebug() << "hnode_gui: <ConfigParser> ERROR: Unknown section in config file!";
      return true;
     }
    }
      ...
      if (opts[0].trimmed()=="IN") {
       ...
       if (opts2.size()==4) {
        ...
        if ((!(conf->commPort >= 65000 && conf->commPort < 65500)) || // Simple port validation
         ...
         qDebug() << "hnode_gui: <ConfigParser> <NET> ERROR: Invalid (uplink) hostname/IP/port settings!";
         return true;
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <NET> ERROR: Invalid (uplink) count of NET|IN params!";
        return true;
       }
       qDebug() << "hnode_gui:" << conf->ipAddr << conf->commPort << conf->strmPort << conf->cmodPort;
       ...
       if (opts2.size()==4) {
        ...
        if ((!(conf->svrCommPort >= 65000 && conf->svrCommPort < 65500)) || // Simple port validation
         ...
         qDebug() << "hnode_gui: <ConfigParser> <NET> ERROR: Invalid (downlink) hostname/IP/port settings!";
         return true;
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <NET> ERROR: Invalid (downlink) count of NET|OUT params!";
        return true;
       }
       qDebug() << "hnode_gui:" << conf->svrIpAddr << conf->svrCommPort << conf->svrStrmPort << conf->svrCmodPort;
      } else {
       qDebug() << "hnode_gui: <ConfigParser> <NET> ERROR: Invalid hostname/IP(v4) address!";
       return true;
      }
     }
    } else {
     qDebug() << "hnode_gui: <ConfigParser> <NET> ERROR: No parameters in section!";
     return true;
    }
     ...
     for (const auto& sect:bufSection) {
      ...
      if (opts[0].trimmed()=="PAST") {
       ...
       if (!(conf->tcpBufSize>=5 && conf->tcpBufSize<=20)) {
        qDebug() << "hnode_gui: <ConfigParser> <BUF> ERROR: PAST not within inclusive (5,20) seconds range!";
        return true;
       }
      } else {
       qDebug() << "hnode_gui: <ConfigParser> <BUF> ERROR: Invalid section command!";
       return true;
      }
     }
    } else {
     qDebug() << "hnode_gui: <ConfigParser> <BUF> ERROR: No parameters in section!";
     return true;
    }
    ...
    if (!commResponse.isEmpty()) qDebug() << "hnode_gui: <ConfigParser> Daemon replied:" << commResponse;
    else qDebug() << "hnode_gui: <ConfigParser> (Timeout) No response from master node!";
    ...
    if (!commResponse.isEmpty()) qDebug() << "hnode_gui: <Config> Daemon replied:" << commResponse;
    else qDebug() << "hnode_gui: <Config> No response or timeout.";
    sList=commResponse.split("\n"); ChnInfo chn;
    ...
    for (int chnIdx=0;chnIdx<sList.size();chnIdx++) { // Individual CHANNELs information
     ...
     chn.interElec.resize(0); for (int idx=0;idx<sList2[7].toInt();idx++) chn.interElec.append(sList2[8+idx].toInt());
     chn.interActive.resize(conf->ampCount); for (int ampIdx=0;ampIdx<conf->ampCount;ampIdx++) chn.interActive[ampIdx]=false;
      ...
      if (opts[0].trimmed()=="ADD") {
       ...
       if (opts2.size()==4) {
        ...
        if (!(colR>=0 && colR<256) || !(colG>=0 && colG<256) || !(colB>=0 && colB<256) || !(colA>=0 && colA<256)) {
         qDebug() << "hnode_gui: <ConfigParser> <PLT> ERROR: Invalid RGBcolor parameter!";
	 return true;
        } else { // Add to the list of recognized colors.
	 conf->rgbPalette.append(QColor(colR,colG,colB,colA));
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <PLT> ERROR: Invalid count of RGBcolor parameters!";
	return true;
       }
      } else {
       qDebug() << "hnode_gui: <ConfigParser> <PLT> ERROR: Invalid section command!";
       return true;
      }
     }
    } else {
     qDebug() << "hnode_gui: <ConfigParser> <PLT> ERROR: No parameters in section!";
     return true;
    }
	...
	//qDebug() << conf->erpRejBwd << conf->erpAvgBwd << conf->erpAvgFwd << conf->erpRejFwd;
        if (!(conf->erpRejBwd>=-2000 && conf->erpRejBwd<=2000 && conf->erpAvgBwd>=-2000 && conf->erpAvgBwd<=2000 &&
         ...
         qDebug() << "hnode_gui: <ConfigParser> <EVT> ERROR: Invalid INTERVAL parameter!";
         return true;
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <EVT> ERROR: Invalid count of INTERVAL parameters!";
        return true;
       }
       ...
       if (opts2.size()==3) {
        if (!(evtNo>0 && evtNo<256) || (evtName.size()>100) || // max 100 chars for name should be more than enough?
         ...
         qDebug() << "hnode_gui: <ConfigParser> <EVT> ERROR: Invalid EventNo parameter!";
         return true;
        } else { // Add to the list of pre-recognized events.
	 conf->events.append(Event(evtNo,evtName,evtColIdx));
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <EVT> ERROR: Invalid count of STIM parameters!";
        return true;
       }
      } else {
       qDebug() << "hnode_gui: <ConfigParser> <EVT> ERROR: Invalid section command!";
       return true;
      }
     }
    } else {
     qDebug() << "hnode_gui: <ConfigParser> <EVT> ERROR: No parameters in section!";
     return true;
    }
    ...
    if (headSection.size()>0) {
	 ...
	 // std::cout << "\n";
         ...
         qDebug() << "hnode_gui: <ConfigParser> <HEAD> <GIZMO> ERROR: filename error!";
         return true;
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <HEAD> <GIZMO> ERROR: while parsing parameters!";
        return true;
       }
       ...
       if (opts2.size()==2) {
        ...
        if (!(headNo>=0 && headNo<(int)conf->ampCount)) { // ampCount should be determined by now.
         qDebug() << "hnode_gui: <ConfigParser> <HEAD> <SCALP|SKULL|BRAIN> ERROR: head# overflow!";
         return true;
        } else {
         if (opts[0].trimmed()=="SCALP") {
          ...
          qDebug() << "hnode_gui: <ConfigParser> <HEAD> <SCALP|SKULL|BRAIN> ERROR: while parsing parameters!";
          return true;
         }
        }
       }
      ...
      if (opts[0].trimmed()=="CTRL") {
       ...
       if (opts2.size()==4) {
        ...
        if ((!(conf->guiCtrlX >= -4000 && conf->guiCtrlX <= 4000)) ||
         ...
         qDebug() << "hnode_gui: <ConfigParser> <GUI> <CTRL> ERROR: Window size settings not in appropriate range!";
         return true;
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <GUI> ERROR: Invalid count of parameters!";
        return true;
       }
       ...
       if (opts2.size()==4) {
        ...
        if ((!(conf->guiAmpX >= -4000 && conf->guiAmpX <= 4000)) ||
         ...
         qDebug() << "hnode_gui: <ConfigParser> <GUI> <STRM> ERROR: Window size settings not in appropriate range!";
	 return true;
        }
       } else {
        qDebug() << "hnode_gui: <ConfigParser> <GUI> ERROR: Invalid count of parameters!";
	return true;
     ...
     qDebug() << "hnode_gui: <ConfigParser> <GUI> ERROR: No parameters in section!";
     return true;
    }
