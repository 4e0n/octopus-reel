...
class CModThread : public QThread {
 Q_OBJECT
 public:
  explicit CModThread(ConfParam *c=nullptr,unsigned int a=0,QPixmap *cb=nullptr,QObject *parent=nullptr) : QThread(parent) {
   conf=c; ampNo=a; cmBuffer=cb; threadActive=true;
   ...
   while (threadActive) {
    {
     ...
     if (conf->quitPending) {
      ...
      updateBuffer();
      emit updateCModFrame();
     }
     conf->guiPending[ampNo]=false;
     conf->guiUpdating--;
     if (conf->guiUpdating==0) conf->tcpBufTail++;
    }
   }
   qDebug("hnode_cmod_gui: <CModThread> Exiting thread..");
  }
