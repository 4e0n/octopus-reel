...
class AcqDaemon : public QObject {
 ...
 public:
  explicit AcqDaemon(QObject *parent=nullptr,ConfParam *c=nullptr,
   ...
   connect(&(conf->commServer),&QTcpServer::newConnection,this,&AcqDaemon::onNewCommClient);
   connect(&(conf->strmServer),&QTcpServer::newConnection,this,&AcqDaemon::onNewStrmClient);
 ...
 private slots:
  void onNewCommClient() {
   while (conf->commServer.hasPendingConnections()) {
    QTcpSocket *client=conf->commServer.nextPendingConnection();
  ...
  void onNewStrmClient() {
   while (conf->strmServer.hasPendingConnections()) {
    QTcpSocket *client=conf->strmServer.nextPendingConnection();
