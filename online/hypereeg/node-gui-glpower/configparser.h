...
const QString NODE_TEXT="GLPOWER";
 ...
 public:
  ...
  bool parse(ConfParam *conf) { QString commResponse; QStringList sList,sList2;
   ...
   if (!cfgFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    qWarning() << "node-gui-glpower: <ConfigParser> ERROR: Cannot load" << cfgPath;
    return true;
   } else {
    ...
    if (idx1<0) {
     qWarning() << "node-gui-glpower: <ConfigParser> ERROR: NODE section does not exist in config file!";
     return true;
    }
    idx1++;
    ...
    for (int idx=idx1;idx<idx2;idx++) {
     opts=cfgLines[idx].split("#"); opts=opts[0].split("|"); // Get rid off any following comment.
      ...
      qWarning() << "node-gui-glpower: <ConfigParser> ERROR: Unknown section in config file!";
      return true;
     }
    }
      ...
      if (opts[0].trimmed()=="COMP") {
       ...
       if (opts2.size()==2) {
        ...
        if ((!(conf->compPPCommPort >= 65000 && conf->compPPCommPort < 65999))) { // Simple port validation
         qWarning() << "node-gui-glpower: <ConfigParser> <COMP> ERROR: Invalid (serving) hostname/IP/port settings!";
         return true;
        }
       } else {
        qWarning() << "node-gui-glpower: <ConfigParser> <COMP> ERROR: Invalid (serving) count of STRM|IN params!";
        return true;
       }
       qInfo() << "node-gui-glpower:" << conf->compPPIpAddr << conf->compPPCommPort;
      } else if (opts[0].trimmed()=="GLPOWER") {
       opts2=opts[1].split(",");
       if (opts2.size()==2) {
        conf->powCommPort=opts2[1].toInt();
        if (!(conf->powCommPort >= 65000 && conf->powCommPort < 65999)) {
         qWarning() << "node-gui-glpower: <ConfigParser> <COMM> ERROR: Invalid local command port!";
         return true;
        }
       } else {
        qWarning() << "node-gui-glpower: <ConfigParser> <COMM> ERROR: Invalid parameter count!";
        return true;
       }
       qInfo() << "node-gui-glpower:" << conf->compPPIpAddr << conf->compPPCommPort;
      } else {
       qWarning() << "node-gui-glpower: <ConfigParser> <COMP> ERROR: Invalid hostname/IP(v4) address!";
       return true;
      }
     }
    } else {
     qWarning() << "node-gui-glpower: <ConfigParser> <COMP> ERROR: No parameters in section!";
     return true;
    }
      ...
      if (opts[0].trimmed()=="MAIN") {
       ...
       if (opts2.size()==4) {
        ...
        if ((!(conf->guiX >= -4000 && conf->guiX <= 4000)) ||
         ...
         qWarning() << "node-gui-glpower: <ConfigParser> <GUI> <CTRL> ERROR: Window size settings not in appropriate range!";
         return true;
        }
       } else {
        qWarning() << "node-gui-glpower: <ConfigParser> <GUI> ERROR: Invalid count of parameters!";
        return true;
       }
      }
     }
    } else {
     qWarning() << "node-gui-glpower: <ConfigParser> <GUI> ERROR: No parameters in section!";
     return true;
    }
