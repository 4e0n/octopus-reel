...
public:
  explicit TcpThread(ConfParam *c,QObject *parent=nullptr):QThread(parent),conf(c) {
   tcpBufSize=conf->tcpBufSize; tcpBuffer=&conf->tcpBuffer; // -> QVector<TcpSamplePP>
   conf->tcpBufTail=0;
  }
...
protected:
  void run() override {
   ...
   // In PP, the serialized sample size is known/configured
   ...
   sendloop::Params p; p.N=N; p.basePeriod_ns=basePeriod_ns; p.target=uint64_t(2*N);

   // PP is typically “more conservative”: require at least target available
   // Change to N+N/2 if it is to behave like node-acq
   p.minAvailToSend=uint64_t(2*N);

   p.Kp_ns_per_sample=std::max<int64_t>(2000,1000000LL/int64_t(p.target)/2);
   p.Ki_ns_per_sample=std::max<int64_t>(1,p.Kp_ns_per_sample/500);
   ...
   quint64 headSnap=0; quint64 tailAfter=0;
   ...
   auto serializeAndAdvance=[this,N,sz,&headSnap,&tailAfter](char* dst) -> bool {
    quint64 tail0=0;
    {
      ...
      conf->spaceReady.wakeOne(); // must be kept if a produer waiting on space exists
    }
    // Serialize OUTSIDE mutex (faster, like the old PP thread)
    for (int i=0;i<N;++i) {
   ...
   dbgFn=[](uint64_t avail,uint64_t target,int64_t err,int64_t corrNs,int64_t periodNs,int64_t baseNs) {
    ...
    qInfo().noquote() << QString("<TcpPLL-PP> avail=%1 target=%2 fill=%3 err=%4 corrNs=%5 periodNs=%6 baseNs=%7")
...
private:
  ConfParam *conf=nullptr; QVector<TcpSamplePP> *tcpBuffer=nullptr; unsigned int tcpBufSize=0;
};
