...
#include "glheadwindow.h"

const int GLFRAME_REFRESH_RATE=1; // Base refresh rate (seconds)

const int GUI_MAX_AMP_PER_LINE=4; // If >4 then (4,2), otherwise (N,1)

const int GL_REPLY_TIMEOUT_MS=1500;

class GLClient: public QObject {
 Q_OBJECT
 public:
  explicit GLClient(QObject *parent=nullptr,ConfParam *c=nullptr) : QObject(parent) { conf=c; }
  ...
  bool start() { QString commResponse; QStringList sList,sList2;
   ...
   if (!conf->compPPCommSocket->waitForConnected(2000)) {
    qCritical() << "node-gui-glhead: <ConfigParser> Cannot connect to node-comp-pp:" << conf->compPPCommSocket->errorString();
    return true;
   }
   ...
   if (commResponse.isEmpty()) {
    qCritical() << "node-gui-glhead: <ConfigParser> No response from Acquisition Node!";
    return true;
   }
   sList=commResponse.split(",");
   if (sList.size()<9) { // or 13 if you expect full GETCONF payload
    qCritical() << "node-gui-glhead: <ConfigParser> Bad GETCONF reply:" << commResponse;
    return true;
   }
   ...
   if (commResponse.isEmpty()) qCritical() << "node-gui-glhead: <GetChannelListFromDaemon> (TIMEOUT) No response from Node!";
   sList=commResponse.split("\n"); GUIChnInfo chn;
   ...
   connect(pollTimer,&QTimer::timeout,this,&GLClient::slotPoll);

   replyTimer=new QTimer(this);replyTimer->setSingleShot(true);
   connect(replyTimer,&QTimer::timeout,this,&GLClient::slotReplyTimeout);

   connect(&(conf->glCommServer),&QTcpServer::newConnection,this,&GLClient::slotNewCommClient);

   connect(conf->compPPCommSocket,&QTcpSocket::readyRead,this,&GLClient::slotReadyRead);
   connect(conf->compPPCommSocket,&QTcpSocket::disconnected,this,&GLClient::slotDisconnected);

   if (!conf->glCommServer.listen(QHostAddress::Any,conf->glCommPort)) {
    qCritical() << "node-gui-glhead: <Comm> Cannot start command server on port:" << conf->glCommPort;
    return true;
   }
   ...
   conf->curGLData.resize(conf->ampCount); for (auto& c:conf->curGLData) c.resize(conf->chnCount);

   glHeadWindow=new GLHeadWindow(conf); glHeadWindow->show(); // GL Levels Window
   pollTimer->start(conf->glRefreshMs);
 ...
 private slots:
  void slotNewCommClient() {
   while (conf->glCommServer.hasPendingConnections()) {
    QTcpSocket *client=conf->glCommServer.nextPendingConnection();
    conf->glClients.append(client);
    ...
    connect(client,&QTcpSocket::disconnected,this,[this,client]() {
     conf->glClients.removeAll(client);
     client->deleteLater();
    });

    qInfo() << "node-gui-glhead: <Comm> Client connected from" << client->peerAddress().toString();
   }
  }

  void handleCommand(const QString &cmd,QTcpSocket *client) {
   qInfo() << "node-gui-glhead: <Comm> Received command:" << cmd;
   if (cmd==CMD_STATUS) {
    client->write("node-gui-glhead: ready.\n");
   } else if (cmd==CMD_GUI_SHOW) {
    if (glHeadWindow) glHeadWindow->show();
    client->write("node-gui-glhead: gui shown.\n");
   } else if (cmd==CMD_GUI_HIDE) {
    if (glHeadWindow) glHeadWindow->hide();
    client->write("node-gui-glhead: gui hidden.\n");
   } else if (cmd==CMD_GUI_RAISE) {
    if (glHeadWindow) {
     glHeadWindow->show(); glHeadWindow->raise(); glHeadWindow->activateWindow();
    }
    client->write("node-gui-glhead: gui raised.\n");
   } else if (cmd==CMD_GUI_REFRESH) {
    if (glHeadWindow) glHeadWindow->updateGLFrames();
    client->write("node-gui-glhead: refreshed.\n");
   } else if (cmd==CMD_GUI_START) {
    conf->pollingActive=true;
    if (pollTimer && !pollTimer->isActive()) pollTimer->start(conf->glRefreshMs);
    client->write("node-gui-glhead: polling started.\n");
    ...
    if (parts.size()==2) {
     ...
     if (ms>=50 && ms<=5000) {
      ...
      client->write(QString("node-gui-glhead: refresh set to %1 ms\n").arg(ms).toUtf8());
     } else {
      client->write("node-gui-glhead: invalid refresh range (50–5000 ms)\n");
     }
    } else {
     client->write("node-gui-glhead: usage GUISETREFRESH=ms\n");
    }
    ...
    if (parts.size()==2) {
     ...
     if (mode=="MEAN" || mode=="MEDIAN") {
      if (glHeadWindow) glHeadWindow->setPaletteMode(mode);
      client->write(QString("node-gui-glhead: palette set to %1\n").arg(mode).toUtf8());
     } else {
      client->write("node-gui-glhead: use MEAN or MEDIAN\n");
     }
    } else {
     client->write("node-gui-glhead: usage GUIPALETTE=MEAN|MEDIAN\n");
    }
   } else if (cmd==CMD_GUI_STOP) {
    conf->pollingActive=false;
    if (pollTimer && pollTimer->isActive()) pollTimer->stop();
    client->write("node-gui-glhead: polling stopped.\n");
   } else if (cmd==CMD_QUIT) {
    client->write("node-gui-glhead: quitting.\n");
    client->flush();
    requestShutdown(); // graceful shutdown
   } else {
    client->write("node-gui-glhead: unknown command.\n");
   }
  }
  ...
  void slotPoll() {
   ...
   //conf->compPPCommSocket->write(QByteArray(CMD_COMPPP_GETGLLEVELS)+"\n");
   //conf->compPPCommSocket->flush();
   requestPending=true; replyTimer->start(GL_REPLY_TIMEOUT_MS);
   ++pollCounter;
  }
  ...
  void slotReadyRead() {
   ...
   while ((nlIdx=rxBuffer.indexOf('\n'))>=0) {
    ...
    if (vals.size()!=expected) {
     qWarning() << "node-gui-glhead: <Parse> Bad reply size:" << vals.size() << "expected:" << expected;
    ...
    for (unsigned int ampIdx=0;ampIdx<conf->ampCount;ampIdx++) {
     for (unsigned int chnIdx=0;chnIdx<conf->chnCount;chnIdx++) {
      conf->curGLData[ampIdx][chnIdx]=vals[k].toFloat();
      k++;
     }
    }
    requestPending=false;
  ...
  void slotReplyTimeout() {
   if (!requestPending) return;
   qWarning() << "node-gui-glhead: <PollTimeout> No GLLEVELS reply within timeout.";
   requestPending=false;
  ...
  void slotDisconnected() {
   qWarning() << "node-gui-glhead: <Socket> Disconnected from node-comp-pp.";
   requestPending=false;
   rxBuffer.clear();
   if (replyTimer->isActive()) replyTimer->stop();
  }
 ...
 private:
  void requestShutdown() {
   qInfo() << "node-gui-glhead: <Shutdown> Requested.";
   conf->quitPending=true; conf->glCommServer.close();
