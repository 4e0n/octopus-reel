...
const QString NODE_TEXT="GLHEAD";
 ...
 public:
  ...
  bool parse(ConfParam *conf) { QString commResponse; QStringList sList,sList2;
   ...
   if (!cfgFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    qWarning() << "node-gui-glhead: <ConfigParser> ERROR: Cannot load" << cfgPath;
    return true;
   } else {
    ...
    if (idx1<0) {
     qWarning() << "node-gui-glhead: <ConfigParser> ERROR: NODE section does not exist in config file!";
     return true;
    }
    idx1++;
    ...
    for (int idx=idx1;idx<idx2;idx++) {
     opts=cfgLines[idx].split("#"); opts=opts[0].split("|"); // Get rid off any following comment.
      ...
      qWarning() << "node-gui-glhead: <ConfigParser> ERROR: Unknown section in config file!";
      return true;
     }
    }
      ...
      if (opts[0].trimmed()=="COMP") {
       ...
       if (opts2.size()==2) {
        ...
        if ((!(conf->compPPCommPort >= 65000 && conf->compPPCommPort < 65999))) { // Simple port validation
         qWarning() << "node-gui-glhead: <ConfigParser> <COMP> ERROR: Invalid (serving) hostname/IP/port settings!";
         return true;
        }
       } else {
        qWarning() << "node-gui-glhead: <ConfigParser> <COMP> ERROR: Invalid (serving) count of STRM|IN params!";
        return true;
       }
       qInfo() << "node-gui-glhead:" << conf->compPPIpAddr << conf->compPPCommPort;
      } else if (opts[0].trimmed()=="GLHEAD") {
       opts2=opts[1].split(",");
       if (opts2.size()==2) {
        conf->glCommPort=opts2[1].toInt();
        if (!(conf->glCommPort >= 65000 && conf->glCommPort < 65999)) {
         qWarning() << "node-gui-glhead: <ConfigParser> <COMM> ERROR: Invalid local command port!";
         return true;
        }
       } else {
        qWarning() << "node-gui-glhead: <ConfigParser> <COMM> ERROR: Invalid parameter count!";
        return true;
       }
       qInfo() << "node-gui-glhead:" << conf->compPPIpAddr << conf->compPPCommPort;
      } else {
       qWarning() << "node-gui-glhead: <ConfigParser> <COMP> ERROR: Invalid hostname/IP(v4) address!";
       return true;
      }
     }
    } else {
     qWarning() << "node-gui-glhead: <ConfigParser> <COMP> ERROR: No parameters in section!";
     return true;
    }
      ...
      if (opts[0].trimmed()=="MAIN") {
       ...
       if (opts2.size()==4) {
        ...
        if ((!(conf->guiX >= -4000 && conf->guiX <= 4000)) ||
         ...
         qWarning() << "node-gui-glhead: <ConfigParser> <GUI> <CTRL> ERROR: Window size settings not in appropriate range!";
         return true;
        }
       } else {
        qWarning() << "node-gui-glhead: <ConfigParser> <GUI> ERROR: Invalid count of parameters!";
        return true;
       }
      }
     }
    } else {
     qWarning() << "node-gui-glhead: <ConfigParser> <GUI> ERROR: No parameters in section!";
     return true;
    }
