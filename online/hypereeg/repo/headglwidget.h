...
class HeadGLWidget : public QGLWidget {
 Q_OBJECT
 public:
  explicit HeadGLWidget(ConfParam *c=nullptr,unsigned int a=0,QWidget *p=nullptr) : QGLWidget(QGLFormat(QGL::SampleBuffers),p) {
   conf=c;
...
//    if (conf->hwFrameV[ampNo]) glCallList(frame);
//    if (conf->hwGridV[ampNo])  glCallList(grid);
...
//   glPopMatrix();
  ...
  void resizeGL(int width,int height) { int side=qMin(width,height);
   ...
   gluPerspective(CAMERA_FOV,(float)(conf->gl3DGuiW)/(float)(conf->gl3DGuiH),2.,120.);
   glMatrixMode(GL_MODELVIEW);
  }
...
//   float elecR; if (chn==conf->currentElectrode[ampNo]) elecR=eR*3; else elecR=eR;
...
//        glVertex3f(v0c0C,v0c1C,v0c2C); glVertex3f(v1c0C,v1c1C,v1c2C);
