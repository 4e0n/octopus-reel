...
struct TcpCMArray {
 ...
 QByteArray serialize() const {
  ...
  for (const auto& v : cmLevel) {
   if (!v.empty()) out.writeRawData(reinterpret_cast<const char*>(v.data()),static_cast<int>(v.size()));
  }
 ...
 bool deserialize(const QByteArray &ba,size_t chnCountIfV0=0) {
  ...
  for (auto& amp:cmLevel) {
   if (!amp.empty()) in.readRawData(reinterpret_cast<char*>(amp.data()),static_cast<int>(amp.size()));
  }
