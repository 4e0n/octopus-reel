...
class ConfigParser {
 public:
  ...
  void parse(ConfParam *conf,QVector<ChnTopo> *chnTopo) {
   QTextStream cfgStream;
   QStringList cfgLines,opts,opts2,ampSection,netSection,chnTopoSection,guiSection;
   ...
   if (!cfgFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    // Separate AMP, NET, CHNTOPO, GUI parameter lines
    ...
    if (chnTopoSection.size()>0) {
     for (int i=0;i<chnTopoSection.size();i++) {
      opts=chnTopoSection[i].split("=");
      if (opts[0].trimmed()=="APPEND") {
       ...
       if (opts2.size()==4) {
        ...
        if ((!((unsigned)opts2[0].toInt()>0  && (unsigned)opts2[0].toInt()<=512)) || // Channel#
         ...
         qDebug() << "octopus_acqd: <ConfigParser> <CHNTOPO> ERROR: Invalid parameter!";
	 application->quit();
	 ...
	 dummyChnTopo.cmLevel[1]=128.0;		// Reset CM Level
        ...
        qDebug() << "octopus_acqd: <ConfigParser> <CHNTOPO> ERROR: Invalid count of APPEND parameters!";
	application->quit();
       }
      }
     }
    } else {
     qDebug() << "octopus_acqd: <ConfigParser> <CHNTOPO> ERROR: No parameters in section!";
     application->quit();
    }
