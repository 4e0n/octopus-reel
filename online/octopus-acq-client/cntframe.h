...
class CntFrame : public QFrame {
 Q_OBJECT
 public:
  CntFrame(QWidget *p,AcqMaster *acqm) : QFrame(p) {
   parent=p; acqM=acqm; w=parent->width()-9; h=parent->height()-60;
   ...
   scrollBuffer=QPixmap(w,h); resetScrollBuffer();

   acqM->registerScrollHandler(this);
  }

  void resetScrollBuffer() {
   QPainter scrollPainter; QRect cr(0,0,w-1,h-1);
   ...
   // scrollPainter.fillRect(cr,QBrush(Qt::white));
    ...
    scrollPainter.drawLine(0,0,w-1,h-1);
   scrollPainter.end();
  }
  ...
  void updateBuffer() {
   ...
   for (c=0;c<colCount;c++) {
    if (wX[c]<=wn[c]) {
     ...
     scrollPainter.drawLine(wX[c],1,wX[c],h-2);
     if (wX[c]<(wn[c]-1)) {
      scrollPainter.setPen(Qt::black);
      scrollPainter.drawLine(wX[c]+1,1,wX[c]+1,h-2);
     }
    }
   }
   ...
   if (tick) {
    ...
    scrollPainter.drawRect(0,0,w-1,h-1);
    for (c=0;c<colCount;c++) {
     scrollPainter.drawLine(w0[c]-1,1,w0[c]-1,h-2);
     scrollPainter.drawLine(wX[c],1,wX[c],h-2);
    }
   }
   ...
   for (int i=0;i<chnCount;i++) {
    ...
    scrPrvDataX=acqM->scrPrvData[i]; scrCurDataX=acqM->scrCurData[i];
...
//      if (i==0) qDebug("%2.2f",scrCurDataX);
    ...
    *rBuffer=rBuffer->transformed(rMatrix,Qt::SmoothTransformation);
    ...
    for (c=0;c<colCount;c++) {
     scrollPainter.drawLine(wX[c],1,wX[c],h-2);
     if (wX[c]<(w0[c]+15)) scrollPainter.drawPixmap(wn[c]-15,h-104,*rBuffer);
     else scrollPainter.drawPixmap(wX[c]-14,h-104,*rBuffer);
    } delete rBuffer;
   } 
