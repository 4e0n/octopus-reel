...
class HeadGLWidget : public QGLWidget {
 Q_OBJECT
 public:
  HeadGLWidget(QWidget *pnt,RecMaster *rm) :
  ...
  ~HeadGLWidget() {
   ...
   if (p->brainExists) glDeleteLists(brain,10);
   if (p->skullExists) glDeleteLists(skull,9);
   if (p->scalpExists) glDeleteLists(scalp,8);
   glDeleteLists(avgs,7);
   if (p->gizmoExists) glDeleteLists(gizmo,6);
   glDeleteLists(realistic,5);
   glDeleteLists(parametric,4);
   if (p->digExists) glDeleteLists(dig,3);
  ...
  void paintGL() {
   makeCurrent(); // glMatrixMode(GL_MODELVIEW);
    ...
    glPushMatrix();
     ...
     if (p->hwSourceV) glCallList(source);
     if (p->brainExists && p->hwBrainV) glCallList(brain);
     if (p->skullExists && p->hwSkullV) glCallList(skull);
     ...
     if (p->digExists && p->hwDigV) glCallList(dig);
     glEnable(GL_LIGHTING);
 ...
 private:
    ...
    glPushMatrix(); glTranslatef(0.0,0.0,r+ELECTRODE_HEIGHT*3./5.+0.002);
     glPushMatrix();
      glBegin(GL_LINES);
       ...
       zPt=-elecR+2.*elecR*(float)p->slTimePt/(float)p->cp.avgCount;
  ...
  GLuint makeDigitizer() { // Digitizer current vectors
   ...
   glBegin(GL_LINES);
    ...
    x0=p->sty[0]; y0=p->sty[1]; z0=p->sty[2];
    x1=p->xp[0]; y1=p->xp[1]; z1=p->xp[2];
    x2=p->yp[0]; y2=p->yp[1]; z2=p->yp[2];
    x3=p->zp[0]; y3=p->zp[1]; z3=p->zp[2];
  ...
  GLuint makeParametric() { float r,h; // Make parametric electrode/cap set..
   ...
   for (int i=0;i<p->acqChannels.size();i++) {
    ...
    } else qglColor(p->acqChannels[i]->notchColor);
  ...
  GLuint makeRealistic() { float sdx,sdy,sdz,error; // Make realistic set..
   ...
   for (int i=0;i<p->acqChannels.size();i++) {
    ...
    sphereXYZ(p->acqChannels[i]->real[0],
  ...
  GLuint makeAverages() { int evtIndex,chn;
   ...
   for (int i=0;i<p->acqChannels.size();i++) { chn=-1;
    ...
    if (p->elecOnReal) { Vec3 dummyVec,vs;
     if (chn>=0) {
      dummyVec=p->acqChannels[chn]->real;
      vs[0]=dummyVec.sphR();
...
//    glTranslatef(0.,-p->scalpNasion,0.);
  ...
  GLuint makeSource() { // Source list defined in main class
   ...
   for (int i=0;i<p->source.size();i++)
    dipole(p->source[i]->pos,p->source[i]->theta,p->source[i]->phi);
   glDisable(GL_BLEND); glEndList(); return list;
  }
  ...
  QWidget *parent; RecMaster *p; int w,h; QPainter painter;
